...
public partial struct CardInfo
{
    ...
    public CardInfo(Card data, int amount = 1)
    {
        cardID = data.CardID;
        this.amount = amount;
    }
    ...
    public Card data
    {
        get
        {
            ...
            return Card.Cache[cardID];
        }
    }

    public Sprite image => data.artwork;
    public string name => data.name; // Scriptable Card name (name of the file)
    public string level => data.level.ToString();

    public List<Target> acceptableTargets => ((CharacterCard)data).acceptableTargets;
}
